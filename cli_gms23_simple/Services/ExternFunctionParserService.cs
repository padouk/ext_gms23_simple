...
namespace cli_gms23_simple.Services
{
	internal class ExternFunctionParserService : IDisposable
	{
		...
		public async Task<IList<ExternFunction>> Parse(string cppFile)
		{
			...
			var externFunctionRegex = new Regex(@"EXTERN\s+(?<return>double|const\s+char\s*\*)\s*\b(?<name>[a-zA-Z0-9_]+)\s*\((?<arguments>[^\)]*)\)", RegexOptions.IgnoreCase | RegexOptions.Multiline | RegexOptions.CultureInvariant | RegexOptions.IgnorePatternWhitespace | RegexOptions.Compiled);
			var externFunctionArgumentRegex = new Regex(@"(?<type>double|const\s+char\s*\*)\s*\b(?<name>[a-zA-Z0-9_]+)(?:\s*=\s*(?<default>[a-zA-Z0-9_:\""]+))?", RegexOptions.IgnoreCase | RegexOptions.Multiline | RegexOptions.CultureInvariant | RegexOptions.IgnorePatternWhitespace | RegexOptions.Compiled);
